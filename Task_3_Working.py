...
def all_stations_with_lines():  # Calling all the immediate stations with times into list
    ...
    for row in data.values:
        if str(row[3]) != 'nan':  # Ignoring the 'nan' strings
            stations.append(row)  # Added strip() method to station names in order to fix the error of spaces of provided data
    return stations
...
def print_line_path(lines, path):
    ...
    for i in range(len(lines) - 1):
        if lines[i] != lines[i + 1]:
            pr += f" when you reach \"{path[i + 1]}\", change line to \"{lines[i + 1]}\""
        elif len(lines) - 2 == i:
            pr += "."
            break
        if i != len(lines) - 2 and lines[i + 1] != lines[i + 2]:
            pr += ","
    print(f"Start from \"{path[0]}\" in \"{lines[0]}\" line{pr}")
...
path_lines = get_path_lines(shortest_path, all_stations_with_lines(), create_graph)

print_line_path(path_lines, shortest_path)
